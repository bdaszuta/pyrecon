...
@function: Visualization: ns surface profile (FV), paginated.
"""
import math
import sys
sys.path.insert(0, '.')
...
def cell_average(fn, xl, xr, nsub=200):
    dx = (xr - xl) / nsub
    return sum(fn(xl + (k + 0.5) * dx) * dx for k in range(nsub)) / (xr - xl)
...
PV_FAMILY = 'fv'
PV_PREFACE = ['lin_mc2_fv', 'ppmx_fv', 'weno5z_fv']
...
def _discover_methods(suffix, preface):
    ...
    for name, _sw, desc in all_methods:
        if name.endswith(suffix):
            # Clean description: strip trailing " (FV)" / " (PW)"
            label = desc
            for tag in (' (FV)', ' (PW)'):
...
def _render_page(data, methods, page_num, family):
    ...
    nl = '\n'
    fig.suptitle(
        f'Hydro profile: N={N}, rho_c={rho_c_hydro:.2e}, '
        f'floor={floor:.1e}{nl}'
    ...
    ax2.set_title(f'Reconstruction error vs position\\n'
    ...
    # Colour by undershoot severity
    colors_surf = []
    for uz in undershoot_zl:
        if uz > 1e-6:
            colors_surf.append('#8B0000')
        elif uz > 1e-10:
            colors_surf.append('#d62728')
        elif uz > 0:
            colors_surf.append('#ff7f0e')
        else:
            colors_surf.append('#2ca02c')

    ax3a.barh(y_pos, errors_surf, color=colors_surf,
    ...
    ax3b.barh(y_pos, errors_full, color=colors_surf,
              edgecolor='black', alpha=0.85)
    ...
    ax3c.barh(y_pos, undershoot_plot, color=colors_surf,
              edgecolor='black', alpha=0.85)
    ...
    fig3.suptitle(
        f'Reconstruction error & undershoot '
...
def main(pages=None):
    """Generate ns_surface_profile plots for all or specified pages.
    ...
    methods = _discover_methods('_fv', PV_PREFACE)
